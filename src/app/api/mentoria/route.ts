...
export async function POST(req: NextRequest) {
  ...
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:560px;margin:0 auto;padding:24px;color:#1A1A1A">
      ...
      <p><strong>Data preferida:</strong> ${dataFormatada} às ${horario}</p>
      <p><strong>IAs usadas no dia a dia:</strong> ${ferramentas}</p>
      <p><strong>Usa Claude:</strong> ${usaClaude}</p>
      <p><strong>Nível de experiência:</strong> ${nivel}</p>
      <p><strong>Pode aumentar créditos de IA para a sessão:</strong> ${creditos}</p>
      <p><strong>Objetivo da mentoria:</strong><br/>${objetivo}</p>
      <p><strong>Projeto atual / o que já tentou:</strong><br/>${projetoAtual}</p>
